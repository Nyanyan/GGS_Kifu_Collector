...
def test_duplicate_move_not_double_registered() -> None:
    ...
    assert state.append_live_move(MoveEvent(move="f5", color="black"))
    assert not state.append_live_move(MoveEvent(move="f5", color="black"))
    assert [move.move for move in state.move_events] == ["f5"]

    state.merge_snapshot_moves(
        [
            MoveEvent(move="f5", color="black"),
            MoveEvent(move="d6", color="white"),
        ]
    )
